...
Item {
	...
	onButtonCaptionsChanged: {
		...
		for (var i = 0; i < buttonCaptions.length; i++) {
			...
			var button = component.createObject(container, {"caption": buttonCaptions[i], "elementID": i});
		...
		if (addBackButton) {
			var component = Qt.createComponent("Button.qml");
			var button = component.createObject(container, {"caption": backButtonCaption, "elementID": buttons.length, "onClicked": goBack()});
